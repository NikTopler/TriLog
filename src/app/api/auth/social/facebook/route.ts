...
export async function GET() {
    ...
    return NextResponse.redirect(`https://www.facebook.com/v17.0/dialog/oauth?client_id=${appId}&redirect_uri=${redirectUri}&state=${state}`, {
        headers: {
            'Set-Cookie': `${AUTH_FACEBOOK_STATE_COOKIE_KEY}=${state}; Path=${PATHS.api.auth.social.facebook}; HttpOnly;${process.env.PRODUCTION ? ' Secure; ' : ' '}SameSite=Strict; Expires=${new Date(Date.now() + AUTH_STATE_TOKEN_EXPIRATION).toUTCString()}`
        }
    });
