...
export async function GET() {
    ...
    return NextResponse.redirect(`https://github.com/login/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&state=${state}&scope=${scope}`, {
        headers: {
            'Set-Cookie': `${AUTH_GITHUB_STATE_COOKIE_KEY}=${state}; Path=${PATHS.api.auth.social.github.login}; HttpOnly;${process.env.PRODUCTION ? ' Secure; ' : ' '} Expires=${new Date(Date.now() + AUTH_STATE_TOKEN_EXPIRATION).toUTCString()}`
        }
    });
